...
def test_find_latest_URLs():
    s = Scrape(dummy_main_url)
    s.get_request_object()
    s.get_web_page()
    s.soupify_webpage()
    result = find_latest_URLs(s.soup)
...
def test_get_todays_date():
    assert get_todays_date() == '2020/6/14'
...
def test_get_url_for_section_and_date():
    assert get_url_for_section_and_date('technology', '2020/6/02') == dummy_section_date_url

def test_find_URLs_by_section():
    s = Scrape(dummy_section_url)
    s.get_request_object()
    s.get_web_page()
    s.soupify_webpage()
    result = find_URLs_by_section_or_date(s.soup)
...
def test_find_URLs_by_section_or_date():
    s = Scrape(dummy_section_date_url)
    s.get_request_object()
    s.get_web_page()
    s.soupify_webpage()
    result = find_URLs_by_section_or_date(s.soup)
